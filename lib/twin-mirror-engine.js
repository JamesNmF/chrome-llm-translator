 ...
 * 1. 📐 物理等高水平对齐 (默认推荐，彻底消灭停滞卡死，左右两边每一行绝对平齐，同轴极速翻滚)
 * 2. 🔗 DOM 语义标签与超链接保留技术 (HTML Masking & Re-hydration，100% 保留蓝色链接、加粗、边框与可点击交互)
 * 3. 🎯 支持随时在顶部切换为动态锚点插值或线性比例模式
 */
...
export class TwinMirrorEngine {
  ...
  async activate() {
    ...
    topBar.innerHTML = `
      ...
      <div style="display: flex; align-items: center; gap: 4px;">
        ...
        <select id="twin-align-select" style="background: #1e293b; border: 1px solid #475569; color: #fff; padding: 2px 6px; border-radius: 4px; font-size: 11px; outline: none; cursor: pointer;">
          <option value="spacer" selected>📐 物理等高平齐 (极速平稳·推荐)</option>
          <option value="anchor">🎯 动态锚点插值</option>
          <option value="linear">⚡ 线性等比滚动</option>
        </select>
      </div>

      <span id="twin-trans-progress" style="color: #10b981; font-weight: 600; font-size: 11px;">● 正在保留格式翻译...</span>
    ...
    // 12. 执行右侧镜像的 DOM 语义保留流式翻译替换
    await this.translateCloneWithDOMPreservation(rightContent, topBar.querySelector('#twin-trans-progress'));
  ...
  bindAdaptiveSyncScroll() {
    const handleScroll = (sourcePane, targetPane, isFromLeft) => {
      ...
      if (sourceScroll <= 2) {
        ...
        // 📐 物理等高模式：左右高度已拉平，直接采用最丝滑稳定的 1:1 同轴同步
  ...
  /**
   * ⭐️ 核心黑科技：DOM 标签骨架与超链接保留流式翻译 (HTML Masking & Re-hydration)
   */
  async translateCloneWithDOMPreservation(root, progressBadge) {
    ...
    const worker = async () => {
      while (queue.length > 0 && this.isActive) {
        ...
        while (queue.length > 0 && batch.length < 4 && totalLen < 800) {
                ...
                batch.forEach((item, idx) => {
                  ...
                  if (item.el) {
                    if (item.subTags.length > 0) {
                      ...
                      // 将清理后的 HTML 赋给段落，超链接与样式 100% 毫发无损！
    ...
    if (progressBadge && this.isActive) {
      progressBadge.textContent = '● 100% 格式保留翻译完成';
    }
  }
