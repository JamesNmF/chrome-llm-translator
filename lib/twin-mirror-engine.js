...
export class TwinMirrorEngine {
  ...
  collectAllTranslatableNodes(root) {
    ...
    rawBlocks.forEach(el => {
      if (el.closest('.html5-video-player, .ytp-chrome-bottom, .ytp-chrome-controls, .ytp-button, .ytp-popup, .bpx-player-control-wrap, .vjs-control-bar, video, audio')) {
        return;
      }
  ...
  deactivate() {
    ...
    // 释放并解绑所有局部子滚动条监听器
    if (this.scrollerPairs && this.scrollerPairs.length > 0) {
      this.scrollerPairs.forEach(p => {
        if (p.left && p.onLeftScroll) p.left.removeEventListener('scroll', p.onLeftScroll);
        if (p.right && p.onRightScroll) p.right.removeEventListener('scroll', p.onRightScroll);
        if (p.left) p.left.removeAttribute('data-llm-scroller-id');
      });
      this.scrollerPairs = [];
    }

    // 彻底清除所有左侧原网页 DOM 上临时垫高的 minHeight 样式与属性
    if (this.anchorPairs && this.anchorPairs.length > 0) {
      this.anchorPairs.forEach(pair => {
        if (pair.leftEl) {
          pair.leftEl.style.minHeight = '';
          pair.leftEl.removeAttribute('data-llm-anchor-id');
        }
      });
      this.anchorPairs = [];
    }
