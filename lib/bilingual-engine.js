...
export class BilingualEngine {
  constructor({ settings, onUpdate = () => {} }) {
    ...
    this.state = 'origin'; // 'origin' | 'dual' | 'side_by_side' | 'translation'
  ...
  /**
   * 切换全网页双语状态 (四态循环: origin -> dual -> side_by_side -> translation -> origin)
   */
  async toggleFullPage() {
    if (this.state === 'origin') {
      return this.setViewState('dual');
    } else if (this.state === 'dual') {
      return this.setViewState('side_by_side');
    } else if (this.state === 'side_by_side') {
  ...
  /**
   * 精确设置视图模式 (origin / dual / side_by_side / translation)
   */
  async setViewState(viewMode) {
    ...
    } else if (viewMode === 'dual' || viewMode === 'side_by_side') {
      this.translatedMap.forEach((record, el) => {
        if (record.originText) {
          this.applyTextToElement(el, record.originText);
        }
        if (record.wrapper) {
          record.wrapper.style.display = viewMode === 'side_by_side' ? 'inline-block' : 'block';
        }
      });
    }
  ...
  isIgnoredElement(el) {
    ...
    // 严格保护视频播放器控件与图标 (YouTube / Bilibili / HTML5 Player)
